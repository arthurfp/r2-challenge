...
func TestOrderRepository_Save_And_ListByUser_ReturnsItems(t *testing.T) {
    ...
    if err != nil { t.Fatalf("save order: %v", err) }
    if saved.ID == "" { t.Fatalf("expected saved order to have ID") }
    ...
    if len(list) == 0 { t.Fatalf("expected at least one order in list") }
    ...
    if found == nil { t.Fatalf("saved order not found in list") }
    if len(found.Items) != 1 { t.Fatalf("expected 1 item, got %d", len(found.Items)) }
    if found.Items[0].ProductID != productID { t.Fatalf("unexpected product id: %s", found.Items[0].ProductID) }
    if found.Items[0].Quantity != 1 { t.Fatalf("unexpected quantity: %d", found.Items[0].Quantity) }
    if found.Items[0].PriceCents != 1234 { t.Fatalf("unexpected price: %d", found.Items[0].PriceCents) }
    ...
    if !containsJSONKey(payload, "items") { t.Fatalf("payload missing items key: %s", string(payload)) }
}
